...
(function($, document, window, undefined) {
    ...
    Plugin.prototype = {
        ...
        _init: function() {
            ...
            var selected_value = Number(this._date.selectedYear)+'-'+(Number(this._date.selectedMonth)+1)+'-'+Number(this._date.selectedDay);
            ...
            if ($target.closest(this.$inputIcon).length === 0 && $target.closest(this.$picker).length === 0 && $target.closest(this.$el).length === 0 && this.options.alwaysShow === false) {
                ...
                if (!this.isMobile) {
                    if (this.selected === true && this.options.alwaysShow === false && this.options.onceClick === true) {
                        this.hide();
                        //여기에 날짜 값 받아오는거 있으면 될듯
                        var selected_value = Number(this._date.selectedYear)+'-'+(Number(this._date.selectedMonth)+1)+'-'+Number(this._date.selectedDay);

                        var today = new Date();
                        var dateArray = selected_value.split("-");

                        var dateObj = new Date(dateArray[0], Number(dateArray[1])-1, dateArray[2]);

                        var betweenDay = parseInt((today.getTime() - dateObj.getTime())/1000/60/60/24)*-1;


                        //
                        var this_id = this.$el.attr('id');
                        $('#'+this_id).text(selected_value);
                        console.log(selected_value);
                        $('.upload_error').hide();
                        ...
                        if(get_diff>100){
                            $('#pue_close').text('판매 기간이 '+get_diff+'일 입니다. 판매 기간이 100일 이상인 상품은 등록할 수 없습니다. 종료일을 적절히 설정해주시고, 종료일 이전에 종료 날짜를 다시 수정해주세요.').show();
                            return false;
                        }
