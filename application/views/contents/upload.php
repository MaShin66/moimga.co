...
<div class="cont-padding">
	...
	<form method="post" action="/contents/upload<?php if($write_type=='modify') {echo '?write=modify&id='.$result['contents_id'];}?>" name="board" id="contents_form" enctype="multipart/form-data">
        ...
        <div class="">
            ...
            <select name="category_id" class="form-control">
                <?php foreach ($cate_list as $key=>$item){?>
                    <option value="<?=$item['contents_category_id']?>" <?php if($write_type=='modify'&&($result['category_id']==$item['contents_category_id'])){echo 'selected';} ?> ><?=$item['title']?></option>
                <?php }?>
		...
		<?php if($write_type=='modify'){?>
			<input type="hidden" value="<?=$result['contents_id']?>" name="contents_id">
		<?php } ?>
            ...
            <div class="row">
                <div class="col-lg-3 col-md-3 col-sm-12">
                    <?php if($write_type=='modify'){
                        ...
                        <?php }else{ ?>
                            <img class="upload_thumbs_sm" src="/www/thumbs/contents/basic.jpg" alt="콘텐츠 섬네일">
                        <?php }
                    }else{?>
                        <img class="upload_thumbs_sm" src="/www/thumbs/contents/basic.jpg" alt="콘텐츠 섬네일">
                    <?php }?>
