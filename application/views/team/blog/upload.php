...
$action_url = '/'.$at_url.'/blog/upload';
$btn_text = '등록하기';
$checked = null;
if(!is_null($write_type)){
    $team_blog_id = $this->uri->segment(4);
    $btn_text = '수정하기';
    $action_url = $action_url.'/'.$team_blog_id.'?type=modify';
}
if($result['status']=='on'){
    $checked = 'checked';
}
...
<form method="post" action="<?=$action_url?>" id="team_blog_form" enctype="multipart/form-data">
    <div class="">
        포스팅 제목
        ...
        <div class="row">
            <div class="col-lg-3 col-md-3 col-sm-12">
                <?php if($write_type=='modify'){
                    ...
                    <?php }else{ ?>
                        <img class="upload_thumbs_sm" src="/www/thumbs/team_blog/basic_thumbs.jpg" alt="프로그램 섬네일">
                    <?php }
                }else{?>
                    <img class="upload_thumbs_sm" src="/www/thumbs/team_blog/basic_thumbs.jpg" alt="프로그램 섬네일">
                <?php }?>
