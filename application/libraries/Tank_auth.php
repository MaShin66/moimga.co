...
class Tank_auth
{
	 ...
	 * @return	array
	 */
	function create_user($username, $email, $password, $email_activation,$realname)
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
            ...
            //username 을 email앞쪽으로 하기로함..
            $email_array = explode('@', $email);
			$data = array(
                'username'	=> $email_array[0],//$username,
    ...
    function create_sns_user($username, $email, $email_activation, $sns_type,$unique_id)
    {
        ...
        $user_temp_info = $this->ci->user_model->get_user_info_by_email($email);
        if($user_temp_info['email']!=null&&$user_temp_info['sns_type']==null){
...
//                $this->clear_login_attempts($user->email);
	 ...
	 * @return	array
	 */
	function set_new_email($new_email) // 비밀번호 검사안함.(sns는 비밀번호 없어서 ) 하는거는 set_new_email_org() 로 가야함
	{
		$user_id = $this->ci->session->userdata('user_id');
    ...
    function set_new_email_org($new_email, $password)
    {
        $user_id = $this->ci->session->userdata('user_id');
